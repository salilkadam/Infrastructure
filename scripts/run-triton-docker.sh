...
# Optional migration from old path if present
if [ -d "/mnt/ssd/models/repo" ] && [ -z "$(ls -A "${MODEL_ROOT}/repo" 2>/dev/null)" ]; then
  echo "Migrating existing models from /mnt/ssd/models/repo to ${MODEL_ROOT}/repo ..."
  sudo rsync -a --delete /mnt/ssd/models/repo/ "${MODEL_ROOT}/repo"/ || true
  sudo chown -R "$USER":"$USER" "${MODEL_ROOT}/repo" || true
fi
